...
if (process.env.KAY_PHASE_D_POSTGRES_TESTS !== "true") {
  throw new Error("KAY_PHASE_D_POSTGRES_TESTS=true is required");
}
import { acquirePhaseDLease, acknowledgeBriefing, completeCommitment, evaluatePhaseD } from "./kayPhaseDService";

const marker = kaySyntheticMarker("KAY_PHASE_D_TEST");
let fixtureEmployeeId = 0;
let fixtureLeadId = 0;

before(async () => {
  const schema = await pool.query(`SELECT
    to_regclass('public.kay_runtime_state') IS NOT NULL runtime_state,
    to_regclass('public.kay_commitments') IS NOT NULL commitments,
    to_regclass('public.kay_promises') IS NOT NULL promises,
    to_regclass('public.kay_manager_reviews') IS NOT NULL reviews,
    to_regclass('public.kay_internal_briefings') IS NOT NULL briefings`);
  assert.deepEqual(schema.rows[0], {
    runtime_state: true, commitments: true, promises: true, reviews: true, briefings: true,
  }, "Phase D schema must be pre-provisioned");
  const employee = await pool.query(`INSERT INTO users(username,password,is_admin,role,is_active)
    VALUES($1,'x',false,'sub_agent',true) RETURNING id`, [`${marker}:employee`]);
  fixtureEmployeeId = Number(employee.rows[0].id);
  const lead = await pool.query(`INSERT INTO crm_leads(lead_source,full_name,status,assigned_to,notes)
    VALUES('manual',$1,'new',$2,$1) RETURNING id`, [marker, fixtureEmployeeId]);
  fixtureLeadId = Number(lead.rows[0].id);
});
after(async () => {
  if (fixtureLeadId) {
    await pool.query(`DELETE FROM kay_lead_status_history WHERE lead_id=$1`, [fixtureLeadId]);
    await pool.query(`DELETE FROM crm_leads WHERE id=$1 AND notes=$2`, [fixtureLeadId, marker]);
  }
  if (fixtureEmployeeId) {
    await pool.query(`DELETE FROM users WHERE id=$1 AND username=$2`, [fixtureEmployeeId, `${marker}:employee`]);
  }
  await pool.end();
});

async function employeeId() {
  assert.ok(fixtureEmployeeId, "Phase D synthetic employee fixture is required");
  return fixtureEmployeeId;
}
...
test("Phase D concurrent promise persistence has exactly one idempotent winner", async () => {
  ...
  try {
    const insert = () => pool.query(`INSERT INTO kay_promises(lead_id,employee_id,promise_text,importance,due_at,idempotency_key,details)
      ...
      ON CONFLICT (idempotency_key) DO NOTHING RETURNING id`, [fixtureLeadId, employee, key]);
    const [a, b] = await Promise.all([insert(), insert()]);
    assert.equal(a.rowCount! + b.rowCount!, 1);
...
test("Phase D commitment lifecycle persists the configured extension cap without CRM writes", async () => {
  const employee = await employeeId();
  const leadBefore = await pool.query(`SELECT id, assigned_to, status FROM crm_leads WHERE id=$1`, [fixtureLeadId]);
...
test("Phase D evaluator lease permits only one concurrent holder", async () => {
  const previous = await pool.query(`SELECT value FROM kay_runtime_state WHERE key='phase_d_evaluator_lease'`);
  try {
    await pool.query(`INSERT INTO kay_runtime_state(key,value) VALUES ('phase_d_evaluator_lease','{"released":true}'::jsonb)
    ...
    if (previous.rows[0]) await pool.query(`UPDATE kay_runtime_state SET value=$1::jsonb WHERE key='phase_d_evaluator_lease'`, [JSON.stringify(previous.rows[0].value)]);
    else await pool.query(`DELETE FROM kay_runtime_state WHERE key='phase_d_evaluator_lease'`);
  }
});
...
test("Phase D lost lease aborts before any evaluator write", async () => {
  ...
  const previousLease = await pool.query(`SELECT value FROM kay_runtime_state WHERE key='phase_d_evaluator_lease'`);
  ...
  try {
    ...
    await pool.query(`INSERT INTO kay_runtime_state(key,value) VALUES ('phase_d_evaluator_lease',$1::jsonb)
    ...
    if (previousLease.rows[0]) await pool.query(`UPDATE kay_runtime_state SET value=$1::jsonb WHERE key='phase_d_evaluator_lease'`, [JSON.stringify(previousLease.rows[0].value)]);
    else await pool.query(`DELETE FROM kay_runtime_state WHERE key='phase_d_evaluator_lease'`);
