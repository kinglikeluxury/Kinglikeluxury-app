  ...
  try {
    ...
    for (const rec of dueResult.rows) {
      try {
        ...
        } else if (PETRA_COMPANY_ID !== null && rec.developer_company_id === PETRA_COMPANY_ID) {
          const submitResult = await submitLeadToPetra(rec.id, 0, "petra_auto");
          if (submitResult.success) {
            result.submitted++;
            console.log(`[DeveloperRegistration] Auto re-registered to Petra recordId=${rec.id} leadId=${rec.crm_lead_id}`);
          } else {
            result.failed++;
